...
class SessionStore:
  ...
  def read(self, session_id:str) -> dict:
    ...
    db = mysql.connect(**self.db_config)
    cursor = db.cursor()
    query = "SET @@session.time_zone=\"+00:00\";" #only works past sql 8.022
    ...
    elapsed = (datetime.utcnow() - created_at).total_seconds() # created at uses local time, so session time is off
    if elapsed > self.expiry:
      print("Session Expired?" + str(elapsed) + " expire " + str(self.expiry))
